...
$$k= \frac{y_1-y_0}{x_1-x_0} (先设定k的计算方式,下面会用到)$$ 
...
通过这个上面这个公式, 即使 x 不在 x0 到 x1 之间并且 k 也不是介于 0 到 1 之间，这个公式也是成立的。在这种情况下，我们也可以求出 y,这种方法叫作线性外插。但是需要注意的时, 由于假定了 y0 和 k 在 x0 和 x1 之外也是固定的, 但现实情况可能和假定情况不一致,所以外推只是一个估计值。
